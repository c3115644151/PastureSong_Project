...
- [ ] **环境应激**:
    ...
    - [ ] 露天雨天 -> 应激值缓慢增加，出现水花粒子。
- [ ] **疾病传播**:
    - [ ] 使用调试棒 `/ps debug infect` 强制感染一只动物。
    - [ ] 观察是否出现喷嚏粒子 (SNEEZE)。
    - [ ] 等待 30秒，观察周围动物是否被传染。
    - [ ] 玩家靠近感染源，观察 ActionBar 是否出现警告并获得流感 Buff。
- [ ] **自然痊愈**: 将患病动物移至舒适环境 (无负载、低应激)，观察是否随时间自愈。

### 5.4 环境与负载测试
- [ ] **牧场识别**:
    - [ ] 围一圈栅栏，地面全铺泥土/草方块 -> `/ps debug info` 显示 `Enclosure: Valid`。
    - [ ] 放入一块石头 -> 显示 `Enclosure: Invalid`。
- [ ] **负载计算**:
    - [ ] 放入 1 只牛 -> Load 增加 1.5。
    - [ ] 等待排泄 (灰化土出现) -> Load 显著增加。
    - [ ] 达到过载阈值 -> 动物头顶出现黑烟，应激值开始自动上升。
- [ ] **粪便清理**:
    - [ ] 右键灰化土 (使用铲子) -> 变回草方块，掉落棕色染料，播放快乐粒子。
